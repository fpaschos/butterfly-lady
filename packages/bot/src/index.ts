...
export async function startBot(config: BotConfig): Promise<Client> {
  ...
  // Register commands
  const commands: Command[] = [rollCommand, helpCommand, probCommand]
...
async function handleProbButton(interaction: ButtonInteraction): Promise<void> {
  ...
  try {
    ...
    // Apply Ten Dice Rule
    const { applyTenDiceRule, queryProbability, ExplosionMode } =
      await import('@butterfly-lady/core')
 ...
 */
export async function shutdownBot(client: Client, statusChannelId?: string): Promise<void> {
